...
public class SWG {
    ...
    private Cell construct(Sequence s1, Sequence s2, float[][] matrix, float o,
        ...
        ExecutorService exeSrv = Executors.newFixedThreadPool(numberOfThreads);
        List<Callable<Void>> executorList = new ArrayList<>();

        for (int ii = 1, kk = n; ii < m; ii++, kk += n) {

            int i = ii;
            int k = kk;

            executorList.add(() -> {
                float similarityScore, g1, g2, h1, h2, f;
